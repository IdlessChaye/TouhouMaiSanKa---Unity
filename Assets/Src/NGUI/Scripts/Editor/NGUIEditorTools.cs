...
static public class NGUIEditorTools
{
	...
	static public bool DrawButton(string text, params GUILayoutOption[] options)
	{
		return GUILayout.Button(text, options);
	}

	static public bool DrawButton(string text, GUIStyle style, params GUILayoutOption[] options)
	{
		return GUILayout.Button(text, style, options);
	}

	static public void DrawArrayList(string[] list, params GUILayoutOption[] options)
	{
		List<string> l = new List<string>(list);

		l.ForEach((obj) =>
		{
			EditorGUILayout.TextField(obj, options);
		});
	}
	static public void DrawArrayList(List<string> list, params GUILayoutOption[] options)
	{
		int index = 0;
		list.ForEach((obj) =>
		{
			EditorGUILayout.BeginHorizontal();
			EditorGUILayout.LabelField(("Element " + index++).ToString(), GUILayout.Width(120));
			EditorGUILayout.TextField(obj, options);
			EditorGUILayout.EndHorizontal();
		});
	}
